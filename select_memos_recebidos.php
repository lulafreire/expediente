...
$ac=mysql_query("select * from memos_recebidos where emissor = '$destinatario' and resposta = '' order by data asc");
...
echo"<option value='0'>Nenhum</option>";
    while($a=mysql_fetch_array($ac)) 
	{
		...
		echo"<option value='$id_oficio_recebido'>Memorando nº $num, de $ndata (Ass.: $ass)</option>";
	}
	echo"</select>";
?>